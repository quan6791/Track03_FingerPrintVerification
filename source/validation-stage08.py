...
fp = sorted(glob('../data_set/validation_output_07/*.jpg'))
model = Model()

load_npz('../model_step08.npz', model)

if device >= 0:
    model.to_gpu(device)
try:
    makedirs('../data_set/validation_output_08')
...
#     print x.shape
    ...
    y_hat.save('../data_set/validation_output_08/{}'.format(split(fp[i])[1])) # *contents* of the validation_output directory
